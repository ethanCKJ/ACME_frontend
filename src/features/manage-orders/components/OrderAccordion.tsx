...
import {
  StaffOrder,
  StaffOrderDetail,
} from "../../../pages/staff-exclusive/ManageOrderPage";
...
interface OrderAccordionProps {
  ...
  setStaffOrders: React.Dispatch<React.SetStateAction<StaffOrder[]>>;
  index: number;
}
...
function OrderAccordion({
  ...
  const hideDueDate =
    staffOrder.orderState === OrderState.FULFILLED ||
    staffOrder.orderState === OrderState.CANCELLED;
  ...
  const dayDifference = Math.floor(
    (new Date(staffOrder.requiredDate).valueOf() - Date.now().valueOf()) /
      (1000 * 60 * 60 * 24),
  );
  ...
  const sendNewOrderState = async () => {
    try {
      await api.post("/set_order_state", {
        orderId: staffOrder.orderId,
        newState: newState,
      });
      // Update local state instead of refetching all data from the database.
      const newItem = staffOrders.filter(
        (va) => va.orderId != staffOrder.orderId,
      );
  ...
  const StyledKeyValuePair = ({ field, value }: StyledTypographyProps) => (
    <Box sx={{ display: "flex" }}>
      <Typography sx={{ fontWeight: "bold", width: "140px" }}>
        {field + ":"}
      </Typography>
    ...
    <Accordion disableGutters>
      <AccordionSummary expandIcon={<ArrowDropDownIcon />}>
        <Typography
          component="span"
          sx={{ width: "230px" }}
        >{`#${staffOrder.orderId} ${displayName}`}</Typography>
        <Typography
          component="span"
          sx={{ marginLeft: "20px" }}
        >{`Due ${staffOrder.requiredDate}`}</Typography>
        {hideDueDate ? null : (
          <Typography
            component="span"
            sx={{
          ...
          >
            {`${Math.abs(dayDifference)} ${message}`}
          </Typography>
        )}
      </AccordionSummary>
      <AccordionDetails sx={{ display: "flex", flexDirection: "column" }}>
        ...
        <Box>
          ...
          <StyledKeyValuePair field="Name" value={staffOrder.customerName} />
          <StyledKeyValuePair field="Email" value={staffOrder.email} />
          <StyledKeyValuePair field="Phone" value={staffOrder.phone} />
          <StyledKeyValuePair
            field="Address Line 1"
            value={staffOrder.addressLine1}
          />
          <StyledKeyValuePair
            field="Address Line 2"
            value={staffOrder.addressLine2}
          />
          <StyledKeyValuePair
            field="Address Line 3"
            value={staffOrder.addressLine3}
          />
          <StyledKeyValuePair field="Postcode" value={staffOrder.postcode} />
          <StyledKeyValuePair field="City" value={staffOrder.city} />
          <StyledKeyValuePair
            field="Required Date"
            value={staffOrder.requiredDate.toString()}
          />
          {staffOrder.orderState === OrderState.NOT_READY ||
          staffOrder.orderState === OrderState.READY_TO_SHIP ? (
            <StyledKeyValuePair
              field="Dispatch date"
            ...
            <StyledKeyValuePair
              field="Dispatch date"
              value={
                staffOrder.dispatchDate === null
                  ? "Not yet dispatched "
                  : staffOrder.dispatchDate.toString()
              }
            />
          )}
        </Box>
          ...
          <FormControl variant="standard" sx={{ m: 1, minWidth: 200 }}>
            <Select
              ...
              onChange={(e: SelectChangeEvent) =>
                setNewState(
                  OrderState[e.target.value as keyof typeof OrderState],
                )
              }
            >
              <MenuItem value={OrderState.NOT_READY}>Not ready</MenuItem>
              <MenuItem value={OrderState.READY_TO_SHIP}>
                Ready to ship
              </MenuItem>
