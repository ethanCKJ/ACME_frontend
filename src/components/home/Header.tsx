        ...
        <Toolbar sx={{height: "60px"}}>
          <Box display={"flex"} justifyContent={"space-between"} flexGrow={1}>
            ...
            <Box sx={{display: "flex", marginLeft: "0px", marginRight: "auto"}}>
              {buttonInfo.map(({href, body, width}, idx) => (
                  <Button key={idx} variant="text" color="inherit" sx={{
                    height: "60px",
                    fontSize: "18px",
                    width: {width},
                    textAlign: "center",
                    lineHeight: "20px"
                  }} href={href}>{body}</Button>))}
            </Box>
            <ButtonGroup>
              ...
              {authenticated ?
                  <Button variant="outlined" color="inherit" endIcon={<AccountBoxIcon/>}
                          sx={{width: "120px"}} onClick={logout}>My Account</Button> :
                  <Button variant="outlined" color="inherit" endIcon={<AccountBoxIcon/>}
                          sx={{width: "120px"}} href={"/login"}>Login</Button>}
            </ButtonGroup>
          </Box>
        </Toolbar>
      </AppBar>
