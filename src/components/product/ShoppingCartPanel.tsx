...
const CartItem = ({productName, imageName, quantity, setQuantity, category}: CartItemProps) => {
...
function ShoppingCartPanel() {
  ...
  return (
      <Box sx={{minWidth: "200px", bgcolor: "ivory", padding: "10px 2px 2px 0px"}}>
        ...
        {cartContent.map((value) => <CartItem key={value.productId} productName={value.productName}
                                                   imageName={value.imageName}
                                                   quantity={value.quantity} category={value.productCategory}
                                                   setQuantity={(newQuantity: number) => adjustQuantityById(value.productId, newQuantity)}/>)}
      </Box>
  )
}
